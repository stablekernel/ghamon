...
The configuration file is a simple text file with one repository per line, in the format `<owner>/<name>`. Blank lines and lines starting with `#` (comment marker) are ignored.
...
The TUI layout consists of a header, a main content area, and a footer. The header displays title, refresh rate, and a progress bar. The content area is divided into columns for repository, workflow name, and status. The footer provides instructions for quitting the application and refreshing the data manually.
...
* The application must be implemented in Go.
* Usage information must be printed to stdout.
* Build and test tasks must be automated with Mage.
* Mage targets must show command output.
* Testify packages must be used for assertions and mocking in unit tests.
...
```
cmd/
   ghamon/      # main package (entry point)
internal/
   ghamon/      # application package
   tui/         # user interface components
   github/      # GitHub API client and data retrieval components
```
